...
SM64_2_Cutscene cutscene_test = {
    CUTSCENE_ACTOR(0, MODEL_MARIO, 280,851,765),
    CUTSCENE_SCRIPT_END()
};

void cutscene_start(SM64_2_Cutscene cutscene) {
    u32 prev_frame = 0;
    u32 frame = 0;
    u32 temp;
    u32 i;
    ...
    for (i = 0; i < 32; i++) {
        cutscene_actors[i] = spawn_object(gCurrentObject, MODEL_NONE, bhvCutsceneActor);
    }
    while (loop) {
        u8 cmd = (u8)cutscene[0];
        switch (cmd) {
            case 0:
                loop = 0;
                break;
        ...
        if (cutscene_events[i].frame == current_frame) {
            switch (cutscene_events[i].type) {
                case (u8)CUTEV_END:
                    ...
                    break;
                case (u8)CUTEV_BGM:
                    play_music(SEQ_PLAYER_LEVEL, SEQUENCE_ARGS(4, (u16)cutscene_events[i].arg1), 0);
                    break;
                case (u8)CUTEV_SFX:
                    play_sound((u16)cutscene_events[i].arg1, gLakituState.pos);
                    break;
                case (u8)CUTEV_STARTNEW:
                    cutscene_start(*(SM64_2_Cutscene*)cutscene_events[i].arg1);
                    break;
                case (u8)CUTEV_SET_MODEL:
                    obj_set_model(cutscene_actors[(u16)cutscene_events[i].arg1], (u16)cutscene_events[i].arg2);
                    break;
                case (u8)CUTEV_SET_ANIM:
                    obj = cutscene_actors[(u16)cutscene_events[i].arg1];
                    geo_obj_init_animation(&obj->header.gfx, &obj->oAnimations[(u16)cutscene_events[i].arg2]);
                    break;
            }
        }
    }
